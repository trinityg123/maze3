...
while not done:
    ...
    if player_x < 0:
        player_x = 0
    elif player_x + player_w > WIDTH:
        player_x = WIDTH - block_w

    if player_y < 0:
        player_y = 0
    elif player_y + player_y > WIDTH:
        player_y = WIDTH - player_y

    player_rect = [player_x, player_y, player_w. player_h]

    for w in walls:
        if intersetcs.rect_rect(player_rect, w):
            if player_vy > 0:
                self.y = 
...
##    for c in coins:
##        pygame.draw.rect(screen, GOLD, c)
